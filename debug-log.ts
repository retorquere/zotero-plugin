...
class DebugLogSender {
  ...
  public register(plugin: string, preferences: string[] = []): void {
    ...
    if (doc) {
      ...
      if (!menupopup) {
        const helppopup = doc.querySelector('menupopup#menu_HelpPopup')
        if (!helppopup) return
        const menupopup = helppopup
      ...
      menuitem.addEventListener('command', event => event.currentTarget && this.send(event.currentTarget))
    }
  }
  ...
  public send(target: EventTarget): void {
    ...
    const plugin: string = elt.getAttribute('label') || '<unknown>'
  ...
  private async rdf(): Promise<string | undefined> {
    if (!this.#zotero) return ''
    ...
    return await translation.translate() // eslint-disable-line @typescript-eslint/no-unsafe-call
  }
}
